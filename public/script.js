...
class BlockchainDebugger {
  ...
  updateDisplay(newState) {
    if (
      JSON.stringify(this.state.mempool) !== JSON.stringify(newState.mempool)
    ) {
    ...
    if (
      JSON.stringify(this.state.wallets) !== JSON.stringify(newState.wallets)
    ) {
    ...
    if (
      JSON.stringify(this.state.balances) !== JSON.stringify(newState.balances)
    ) {
    ...
    if (
      JSON.stringify(this.state.nodes) !== JSON.stringify(newState.nodes) ||
  ...
  step() {
    var port = parseInt(API_CONFIG.baseUrl.split(":")[2]);
    port = 5001 + ((port - 5000) % 4);
    API_CONFIG.baseUrl = `http://localhost:${port}`;
    this.UpdateURL();
  }
  UpdateURL() {
 ...
 handleAddWallet() {
    // send a POST request to the API to add a new wallet 
    fetch(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.addWallet}`, {
        method: "POST",
        }).then((response) => {
            this.logMessage("Wallet added successfully");
            this.refreshData();
            })
        .catch((error) => {
            this.logError("Failed to add wallet:", error);
        });
    }
handleTransaction(event) {
    event.preventDefault();
    const fromWallet = document.querySelector("input[name='from']").value;
    const toWallet = document.querySelector("input[name='to']").value;
    const amount = document.querySelector("input[name='amount']").value;
    const fromData = new FormData();
    fromData.append("from", fromWallet);
    fromData.append("to", toWallet);
    fromData.append("amount", amount);
    
    fetch(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.send}`, {
        method: "POST",
        body: fromData, 
        })
        .then((response) => {
            this.logMessage("Transaction sent successfully");
            this.refreshData();
        })
        .catch((error) => {
  ...
  setupEventListeners() {
    ...
    document
      ...
      .addEventListener("click", () => this.Continue());
  }
}
